...
/**
 * Matches [p], enforcing the transactionality rule (g: Grammar, either suceeds or fails with no side-effects).
 * Hence, [p] is absolved from having to respect the transactionality rule itself.
 */
class Transact (g: Grammar, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 */
class IgnoreErrors (g: Grammar, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 */
class IgnoreErrorsIfSuccessful (g: Grammar, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 * Runs [f] then succeed.
 */
class Perform (g: Grammar, val f: Grammar.() -> Unit): NaiveParser()
{
    init { grammar = g }
 ...
 */
class Log (g: Grammar, val str: String): NaiveParser()
{
    init { grammar = g }
 ...
 */
class Contain (g: Grammar, val failure: () -> String, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 */
class TransactContain (g: Grammar, val failure: () -> String, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 * [CaughtException].
 */
class Catch (g: Grammar, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 */
class CatchContain (g: Grammar, val p: Parser): NaiveParser()
{
    init { grammar = g }
 ...
 */
class Inner (g: Grammar, val gather: Parser, val refine: (String) -> Boolean): NaiveParser()
{
    init { grammar = g }
 ...
 * to [gobble].
 */
class UntilInner (g: Grammar, val terminator: Parser, val refine: (String) -> Boolean): NaiveParser()
{
    init { grammar = g }
